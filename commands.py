...
async def pause_cb(client, query: CallbackQuery):
    ...
    try:
        ...
        await query.message.reply(
            f"⏸ **Pᴀᴜsᴇᴅ Bʏ** » {query.from_user.mention}"
        )
...
async def resume_cb(client, query: CallbackQuery):
    ...
    try:
        ...
        await query.message.reply(
            f"▶️ **Rᴇsᴜᴍᴇᴅ Bʏ** » {query.from_user.mention}"
        )
...
async def skip_cb(client, query: CallbackQuery):
    ...
    await query.message.reply(
        f"⏭ **Sᴋɪᴘᴘᴇᴅ Bʏ** » {query.from_user.mention}"
    )

    await send_now_playing(query.message, music_queues[chat_id][0])
...
async def stop_cb(client, query: CallbackQuery):
    ...
    await query.message.reply(
        f"⏹ **Sᴛᴏᴘᴘᴇᴅ Bʏ** » {query.from_user.mention}"
    )
